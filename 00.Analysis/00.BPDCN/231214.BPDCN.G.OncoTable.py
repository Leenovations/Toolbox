...
#------------------------------------------------------------#
Filtered = Onco_Info[Onco_Info.iloc[:, Onco_Info.columns.get_loc('Classification')] != 'Synonymous']
Filtered = pd.DataFrame(Filtered)
#------------------------------------------------------------#
Sample = sorted(list(set(Filtered.iloc[:, Filtered.columns.get_loc('NGS data')].tolist())))
...
for row in range(0, Filtered.shape[0]):
    ...
    Classification = Filtered.iloc[row, Filtered.columns.get_loc('Classification')]
    Sample = Filtered.iloc[row, Filtered.columns.get_loc('NGS data')]
